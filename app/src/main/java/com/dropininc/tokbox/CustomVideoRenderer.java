...
public class CustomVideoRenderer extends BaseVideoRenderer {
    ...
    private static SaveScreenshotListener saveScreenshotListener;

    public void setSaveScreenshotListener(SaveScreenshotListener saveScreenshotListener) {
    ...
    static class MyRenderer implements GLSurfaceView.Renderer {
        ...
        public MyRenderer(CustomVideoRenderer parent) {
        ...
        public void displayFrame(Frame frame) {
            ...
            if (mCustomVideoRenderer.mSaveScreenshot) {
                Logs.log("CustomVideoRenderer", "Capturing frame....");
                ...
                try {
                    String path = Environment.getExternalStorageDirectory().toString() + "/DropIn/Screenshots/";
                    File folder = new File(path);
                    if (!folder.exists()) {
                        folder.mkdirs();
                    }
                    FileOutputStream fOutputStream = null;
                    String fileName = path + "dropin_capture_" + System.currentTimeMillis() + ".png";
                    File newImage = new File(fileName);
                    fOutputStream = new FileOutputStream(newImage);
                    bmp.compress(Bitmap.CompressFormat.PNG, 100, fOutputStream);
                    fOutputStream.flush();
                    fOutputStream.close();
                    Logs.log("CustomVideoRenderer", "Capturing Screen Done");
                    mCustomVideoRenderer.mSaveScreenshot = false;
                    addImageToGallery(newImage.getAbsolutePath(), mCustomVideoRenderer.mContext);
                    if (saveScreenshotListener != null) {
                        saveScreenshotListener.savePhoto(newImage.getAbsolutePath());
                    }
                } catch (Exception e) {
                    Logs.log(e);
                    return;
        ...
        public void addImageToGallery(final String filePath, Context context) {
            ...
            Logs.log("addImageToGallery", "Save Image To Gallery Done");
        }
    ...
    public void saveScreenshot(SaveScreenshotListener listener) {
        mSaveScreenshot = true;
        this.saveScreenshotListener = listener;
    }
}
